...
const Navbar = () => {
  ...
  const searchData = useRef() as RefObject<HTMLInputElement>;

  const keyDownHandler = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      event.preventDefault();
      setTerm(searchData.current?.value as string);
      navigate("/");
    }
  };
      ...
      <div className="flex items-center justify-between py-2 px-[4rem] max-xl:flex-col shadow-[0_8px_30px_rgb(0,0,0,0.12)] font-customFont">
        ...
        <div className="relative max-xl:w-full max-xl:flex">
          <div className="relative max-xl:w-[80%] mx-auto">
            ...
            <div className="absolute top-3 right-5 cursor-pointer hover:text-[#005D6C] max-xl:top-1">
        ...
        <div className="max-xl:w-full max-xl:flex max-xl:justify-center max-xl:mt-2">
          <button
            className="font-semibold text-[1.5rem] bg-[#005D6C] hover:bg-[#005e6c9a] text-[#ffffff] rounded-full px-3 py-[0.7rem] transition duration-200 max-xl:w-[80%] max-xl:py-2"
